...
The project uses Gradle version catalogs ([libs.versions.toml](./gradle/libs.versions.toml)) for centralized dependency management.

**Key Features:**
- ✅ **Centralized versions**: All dependency versions in one file
- ✅ **Type-safe accessors**: Auto-completion in build scripts
- ✅ **Conflict resolution**: Mapbox duplicate class conflicts resolved
- ✅ **Modern versions**: All dependencies kept up-to-date

**Mapbox Dependency Fix:**
The project includes automatic resolution of Mapbox SDK duplicate class conflicts. See [MAPBOX_DEPENDENCY_FIX.md](./MAPBOX_DEPENDENCY_FIX.md) for details on how we exclude `com.mapbox.common:okhttp` to prevent conflicts with `com.mapbox.common:common:24.0.0`.
...
2. **Configure API Keys**
   ...
   # Mapbox Tokens (Get from https://account.mapbox.com/access-tokens/)
   # Public token (starts with pk.) - For runtime SDK usage
   MAPBOX_ACCESS_TOKEN=pk.YOUR_PUBLIC_TOKEN
   # Secret token (starts with sk.) - For Maven repository access (requires DOWNLOADS:READ scope)
   MAPBOX_DOWNLOADS_TOKEN=sk.YOUR_SECRET_TOKEN
   
   # Firebase
   # Download google-services.json from Firebase Console
   # Place it in app/ directory (currently disabled, can be enabled later)
   ```

   **Note**: `local.properties` is gitignored and will NOT be committed. See `local.properties.example` for reference.

3. **Mapbox Setup**

   **Important**: The project includes dependency conflict resolution for Mapbox SDK. See [MAPBOX_DEPENDENCY_FIX.md](./MAPBOX_DEPENDENCY_FIX.md) for technical details.

   - Sign up at [Mapbox](https://account.mapbox.com/)
   - Create TWO tokens:
     - **Public token** (starts with `pk.`) - For runtime SDK usage
     - **Secret token** (starts with `sk.`) - For Maven repository access (requires DOWNLOADS:READ scope)
   - Add both tokens to `local.properties`:
     ```properties
     MAPBOX_ACCESS_TOKEN=pk.YOUR_PUBLIC_TOKEN
     MAPBOX_DOWNLOADS_TOKEN=sk.YOUR_SECRET_TOKEN
     ```
   - See [MAPBOX_SETUP.md](./MAPBOX_SETUP.md) for complete setup instructions
